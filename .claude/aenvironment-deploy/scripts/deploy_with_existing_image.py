...
1. Configure CLI
2. Initialize environment (config-only mode)
3. Update config.json with existing image
...
def main():
    parser = argparse.ArgumentParser(
        description="Deploy with existing Docker image"
    )
    ...
    parser.add_argument("--image-name", required=True,
                       help="Existing Docker image name")
    ...
    try:
        ...
        print(f"\nStep 2/5: Initializing environment config '{args.env_name}'...")
        ...
        print(f"✓ Environment config initialized at {env_dir}")

        print(f"\nStep 3/5: Updating config with image '{args.image_name}'...")
        ops.update_config_with_image(env_dir, args.image_name)
        print("✓ Config updated with image")

        print("\n⚠ IMPORTANT: Please review the config.json file and customize:")
        print(f"   - {env_dir}/config.json")
